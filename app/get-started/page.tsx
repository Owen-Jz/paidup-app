...
export default function GetStarted() {
  ...
  // processing animation -> create first invoice -> redirect
  useEffect(() => {
      ...
      <main className="ob-main">
        ...
        {step === 2 && (
          <div className="ob-card">
            ...
            <input id="ob-acct" value={connected ? "Cresiolabs sandbox (demo workspace)" : ""} readOnly={connected} placeholder="parent account id" />
            <div className="field-row">
              <div><label htmlFor="ob-cid">Client ID</label><input id="ob-cid" value={connected ? "demo · pre-connected" : ""} readOnly={connected} placeholder="client id" /></div>
              <div><label htmlFor="ob-sec">Client secret</label><input id="ob-sec" type="password" value={connected ? "demo" : ""} readOnly={connected} placeholder="private key" /></div>
            </div>
            <div style={{ marginBottom: 18 }}>
              <span className="demo-pill" onClick={() => setConnected(true)}>⚡ Use demo sandbox (Cresiolabs)</span>
            ...
            <div className="ob-actions">
              ...
              <button className="btn-xl" onClick={() => setStep(3)} disabled={!connected} style={{ opacity: connected ? 1 : 0.5 }}>
                {connected ? "Connected — continue →" : "Connect to continue"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === "processing" && (
          <div className="proc">
            ...
            <h2>Arming your reconciliation engine…</h2>
            {TASKS.map((t, i) => (
